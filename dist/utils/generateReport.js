...
export const generatePDF = (visits, res) => {
...
export const generateCSV = (visits, res) => {
    const fields = [
        { label: 'Technician', value: 'technicianName' },
        { label: 'Site', value: 'siteName' },
        { label: 'Status', value: 'status' },
        { label: 'Step', value: 'currentStep' },
        { label: 'Check-in', value: (row) => new Date(row.checkInTime).toLocaleString() },
        { label: 'Check-out', value: (row) => (row.checkOutTime ? new Date(row.checkOutTime).toLocaleString() : '') },
        { label: 'Location', value: (row) => row.gpsLocation.address || `${row.gpsLocation.lat}, ${row.gpsLocation.lng}` },
    ];
